...
router.get('/daily', async (req, res) => {
  try {
    ...
    try {
      ...
      // Eğer JSON parse edilemezse, default sorular kullan (günde 3 soru)
      questions = {
        questions: [
          {
            id: `q1_${today}`,
            text: "Bugün hangi kategorideki haberleri daha çok okudunuz?",
            category: "Haber Tercihi",
            options: ["Ekonomi 📈", "Spor ⚽", "Teknoloji 💻", "Gündem 📰"]
          },
          {
            id: `q2_${today}`,
            text: "AA Eksen'i hangi cihazdan kullanıyorsunuz?",
            category: "Kullanım",
            options: ["Bilgisayar 💻", "Telefon 📱", "Tablet 📲", "Hepsi 🌐"]
          },
          {
            id: `q3_${today}`,
            text: "Haberleri genellikle günün hangi saatinde okursunuz?",
            category: "Alışkanlık",
            options: ["Sabah 🌅", "Öğlen ☀️", "Akşam 🌆", "Gece 🌙"]
          }
        ]
      };
    }

    // Sadece 3 soru döndür (max limit)
    const limitedQuestions = (questions.questions || questions).slice(0, 3);
