...
router.post('/:podcastId/audio', async (req, res) => {
  try {
    const { podcastId } = req.params;
    const { voice = 'nova' } = req.body;
    
    console.log(`🎙️ Podcast ses dosyası oluşturuluyor: ${podcastId}, Ses: ${voice}`);
    
    // Mock podcast içeriğini al (gerçek uygulamada veritabanından gelecek)
    const mockPodcast = {
      id: podcastId,
      content: `Arkadaşlar, merhaba! Ben bugün sizlere ekonomi dünyasından çok önemli gelişmeleri aktaracağım. 
...
Ve işte, bu kararın piyasalara olan etkileri gerçekten dikkat çekici! Uzmanlar ne diyor, birlikte bakalım...

Arkadaşlar, bugünlük bu kadar. Umarım faydalı olmuştur. Bir sonraki podcast'te görüşmek üzere, kendinize iyi bakın!`
    };
    
    if (!mockPodcast || !mockPodcast.content) {
      return res.status(404).json({
        error: 'Podcast bulunamadı',
        message: 'İstenen podcast içeriği mevcut değil'
      });
    }
    
    // OpenAI TTS ile ses oluştur
    const audioBuffer = await openaiService.generatePodcastAudio(
      mockPodcast.content,
      { 
        voice: voice,
        speed: 1.1 
    ...
    res.set({
      ...
      'Content-Disposition': `attachment; filename="podcast_${podcastId}.mp3"`
    });
    
    console.log(`✅ Ses dosyası gönderiliyor: ${(audioBuffer.length / 1024).toFixed(2)} KB`);
    ...
    res.status(500).json({
      ...
      message: error.message || 'Podcast ses dosyası oluşturulamadı.'
    });
  }
});
